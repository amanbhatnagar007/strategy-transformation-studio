...
class ColumnSchema:
    ...
    def template_df(self, sample_rows: list[dict] | None = None) -> pd.DataFrame:
        if sample_rows:
            return pd.DataFrame(sample_rows)[self.names]
        return pd.DataFrame([{c.name: c.example for c in self.cols}])

    def validate(self, df: pd.DataFrame):
        """Return (clean_df, errors, warnings). Coerces types; never fabricates rows."""
        errors, warnings = [], []
        missing = [c for c in self.names if c not in df.columns]
        if missing:
            errors.append(f"Missing required column(s): {', '.join(missing)}")
            return None, errors, warnings
        df = df[self.names].copy()
        for c in self.cols:
        ...
        before = len(df)
        df = df.dropna(subset=[c.name for c in self.cols if c.kind != "text"])
        if len(df) < before:
            warnings.append(f"{before - len(df)} row(s) with missing numbers were dropped.")
        if df.empty:
...
def _schema_help(schema: ColumnSchema):
    rows = "".join(
        f"<tr><td style='padding:3px 10px 3px 0;color:#c4b5ff'>{c.name}</td>"
    ...
    st.markdown(
        "<div style='font-size:.83rem'><b style='color:#fff'>Required columns</b>"
...
def data_input(schema: ColumnSchema, sample_rows: list[dict], demo_label="Use demo data",
    ...
    if use_demo:
        df, _, _ = schema.validate(pd.DataFrame(sample_rows))
    ...
    try:
        ...
        return None
    df, errors, warnings = schema.validate(raw)
    for w in warnings:
        st.warning(w)
    for e in errors:
        st.error(e)
